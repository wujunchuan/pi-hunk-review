...
function appendExtraInstructions(prompt: string, extraInstructions?: string): string {
	const extra = extraInstructions?.trim();
	if (!extra) return prompt;
	return `${prompt}\n\nAdditional user instructions:\n${extra}`;
}

export function buildReviewPrompt({ cwd, extraInstructions }: PromptOptions): string {
...
Do not modify project files. Do not add comments for purely subjective style preferences. If there is no matching live Hunk session, explain how to start one with \`hunk diff --watch\`. Finish with a short summary of how many comments were added.`;
	return appendExtraInstructions(prompt, extraInstructions);
}

export function buildFixPrompt({ cwd, extraInstructions }: PromptOptions): string {
